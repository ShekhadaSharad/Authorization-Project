    ...
    public class AdminAuthorizController : ControllerBase
    {
        ...
        public async Task<IActionResult> AdminProvideRole(string email, string role)
        {
            ...
            try
            {
                ...
                {
                    Serilog.Log.Error($"HTTP : {Request.Method} : {Request.Path} responded : {Response.StatusCode}. This Role {role} Does Not Exist. login this user : {userName}");
                ...
                {
                    Serilog.Log.Error($"HTTP : {Request.Method} : {Request.Path} responded : {Response.StatusCode}. Failed to add user {user} to the role. {role}. login this user : {userName}");
                    return BadRequest(new Response { Status = "Error", Message = "Failed to add user to the role." });
                }
            }
            catch (Exception ex)
            {
